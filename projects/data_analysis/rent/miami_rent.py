...
for col in unique_col_names[1:-1]:
    df_miami_new[col] = (df_miami[col+'-12'] - df_miami.iloc[:,col_numb])/df_miami.iloc[:,col_numb]*100
    col_quant2 += 1
# add new columns with monthly changes
col_quant3 = col_quant2 # variable to know how many columns after adding
for col in df_miami.columns[col_numb:]:
    df_miami_new[col+' changes'] = (df_miami[col] - df_miami.iloc[:,col_numb])/df_miami.iloc[:,col_numb]*100
    col_quant3 += 1
...
# year = input()
year = '2'
mean = df_miami.iloc[:,col_quant].mean() # finding point to split graphs

### graph with % of monthly changing in prices
## expensive regions
# prepearing data
df_miami_slice = df_miami_new.loc[df_miami.iloc[:,col_quant]>mean] # slicing needed data
...
fig.update_layout(title={'text': "The dynamic of monthly changing average prices of 1br in Miami in expensive regions"}, width=1050, height=600)
fig.show()
...
for region in df_miami_slice['RegionName'].unique():
  fig.add_trace(go.Scatter(y=df_miami_year[region], x=df_miami_year['index'], mode='lines+markers', name=region))
fig.update_layout(title={'text': "The dynamic of monthly changing average prices of 1br in Miami in cheap regions"}, width=1050, height=600)
fig.show()
...
# for region in df_miami_slice['RegionName'].unique():
#   fig.add_trace(go.Scatter(y=df_miami_year[region], x=df_miami_year['index'], mode='lines+markers', name=region))
# fig.update_layout(title={'text': "The dynamic of yearly changing average prices of 1br in Miami in expensive regions"}, width=1000, height=600)
# fig.show()
...
# for region in df_miami_slice['RegionName'].unique():
#   fig.add_trace(go.Scatter(y=df_miami_year[region], x=df_miami_year['index'], mode='lines+markers', name=region))
# fig.update_layout(title={'text': "The dynamic of yearly changing average prices of 1br in Miami in cheap regions"}, width=1000, height=600)
# fig.show()

### graph with simple dynamic
## expensive regions
# prepearing data
df_miami_slice = df_miami.loc[df_miami.iloc[:,col_quant]>mean] # slicing needed data
