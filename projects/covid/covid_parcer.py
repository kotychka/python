...
# header_yest = str(table_yest.find('thead').get_text().replace(',', '').replace('\\n', ' ').split('\\n')).replace('/\\n', '-').replace('/', '-').replace('\\xa0', '')[6:-19].split('\\n')
...
# countries_data = pd.DataFrame([header,countries_today,countries_yest])
	# { 
	# 	'Headers': header_today,
	# 	'Values for today':countries_today,
	# 	'Values for yesterday':countries_yest,
	# })

# countries_data.to_csv('countries.csv', sep=';',index=False, index_label=False)

# with pd.ExcelWriter('CoVid_data.xlsx', mode='a') as writer:  
# 	countries_data.to_excel(writer, sheet_name='Data', index=False)